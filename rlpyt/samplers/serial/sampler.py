...
class SerialSampler(BaseSampler):
    ...
    def initialize(
        ...
        if self.eval_n_envs > 0:  # May do evaluation.
            eval_envs = [self.EnvCls(**self.eval_env_kwargs)
                for j in range(self.eval_n_envs)]
    ...
    def visualize_agent(self, itr, eval_mode="test"):
        if eval_mode == "test":
            return self.eval_test_collector.visualize(itr)
        elif eval_mode == "train":
            return self.eval_train_collector.visualize(itr)
