...
class MinibatchRlBase(BaseRunner):
    ...
    def log_diagnostics(self, itr, traj_infos=None, eval_time=0):
        ...
        if self._eval:
            logger.record_tabular(f'CumuTrainTime',
...
class MinibatchRlEval(MinibatchRlBase):
    ...
    def train(self):
        ...
        with logger.prefix(f"itr #0 "):
            train_traj_infos, train_time = self.evaluate_agent(0)
            self.log_diagnostics(0, train_traj_infos, train_time)
        for itr in range(n_itr):
            with logger.prefix(f"itr #{itr} "):
                ...
                self.store_diagnostics(itr, traj_infos, opt_info)
                if (itr + 1) % self.log_interval_itrs == 0:
                    train_traj_infos, train_time = self.evaluate_agent(itr)
                    self.log_diagnostics(itr, train_traj_infos, train_time)
        self.shutdown()
